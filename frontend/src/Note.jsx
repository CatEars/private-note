...
export const Note = (props) => {
    ...
    if (withinLastFourSeconds(lastCopy)) {
        copied = (
            <div
              className="bg-blue-500 text-white text-sm font-bold px-4 py-3"
            >
    ...
    return (
        <div>
          ...
          <h2 className="">
            You are viewing note: <span className="font-bold">{noteId}</span>
          </h2>
          <p
            className="resize-none my-5 my-2 text-left px-3 content-start border rounded-b shadow bg-white"
          >{note.message}</p>
          <button
          ...
          >Copy</button>
          <p className="my-3">
            Burns up in <span className="font-bold">{humanize(burnIn)}</span>
          </p>
          </div>
        </div>
    );
